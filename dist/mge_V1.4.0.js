...
mge._synth._synthetizer  = {
    play:  function(_frequency, _startTime, _duration, _volume) {
        this._synthPlay(_frequency, _startTime, _duration, _volume)
    }
}
...
mge._camera._worldToScreen = function(_sprite) {
    ...
    if (_sprite._scrollFactor == 0) {
        ...
        _result._scaleXScreen = _sprite._scaleX * mge._camera._zoom
        _result._scaleYScreen = _sprite._scaleY * mge._camera._zoom
    }
   return _result
}
...
mge._timer._update = function () {
    ...
    if (this._mode == 'L') {
        this._progress = (Date.now() - this._startTimestamp) % this._duration / this._duration
    } else {
